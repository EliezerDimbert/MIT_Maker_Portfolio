...
void playSound(String audioFile) {
  ...
  } else if (audioFile == "Rht") {
    sendCommand(CMD_PLAY_FOLDER_FILE, 1, 2);
  } else if (audioFile == "Bck") {
    sendCommand(CMD_PLAY_FOLDER_FILE, 1, 3);
  } else if (audioFile == "Lft") {
    sendCommand(CMD_PLAY_FOLDER_FILE, 1, 4);
  } else if (audioFile == "Arr") {
    sendCommand(CMD_PLAY_FOLDER_FILE, 2, 14);
  } else if (audioFile == "Srt") {
    sendCommand(CMD_PLAY_FOLDER_FILE, 1, 6);
    delay(4000);  //Extra long file so more delay needed
  } else if (audioFile == "KnS") {
    sendCommand(CMD_PLAY_FOLDER_FILE, 2, 15);
  } else if (audioFile == "KnE") {
    sendCommand(CMD_PLAY_FOLDER_FILE, 2, 16);
  } else if (audioFile == "Inv") {
    sendCommand(CMD_PLAY_FOLDER_FILE, 1, 8);
  } else if (audioFile == "Cnfrm") {
    sendCommand(CMD_PLAY_FOLDER_FILE, 1, 7);
  } else {
    Serial.println(F("ERROR: WRONG NAME PASSED THRU playSound"));
    Serial.println("Passes " + audioFile + " thru playSound");
  }
  delay(2000);
}

void playRoom(int roomNum) {
  switch (roomNum) {
    case 0:
    ...
    case 12:
      ...
      break;
    default:
      Serial.println(F("ERROR PLAYING ROOM"));
      break;
  }
  delay(2000);
}

void playWayDwn(int wayDown) {
  switch (wayDown) {
    case 1:
      sendCommand(CMD_PLAY_FOLDER_FILE, 2, 1);
      break;
...
void sendMP3Command(char c) {
  switch (c) {
    ...
    case '1':
      Serial.println("Playing 1,1");
      sendCommand(CMD_PLAY_FOLDER_FILE, 1, 1);
      break;
    case '2':
      Serial.println("Playing 1,2");
      sendCommand(CMD_PLAY_FOLDER_FILE, 1, 2);
      break;
    case '3':
      Serial.println("Playing 1,3");
      sendCommand(CMD_PLAY_FOLDER_FILE, 1, 3);
      break;
    case '4':
      Serial.println("Playing 1,4");
      sendCommand(CMD_PLAY_FOLDER_FILE, 1, 4);
      break;
  }
}
